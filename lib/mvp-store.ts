...
const seedItems: MvpItem[] = seedRecords.map((record, index) => ({
  ...
  descriptor: record[1],
  detail: record[2],
  priceUsdc: Number((3 + index * 0.01).toFixed(3)),
  usageCount: extractNumber(record[2], 12 + index * 7),
  paidRuns: extractNumber(record[1], 4 + index * 3),
  revenueUsdc: Number(((3 + index * 0.01) * (4 + index * 3)).toFixed(3)),
  status: "Active",
  payload: {
    label: record[0],
    descriptor: record[1],
...
function slugify(value: string) {
  const base = value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return base || `crosspay-item-${Date.now()}`;
}
